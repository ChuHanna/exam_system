class RecordsController < ApplicationController
  ...
  def create
    if params[:record].present?
      answers = params[:record].values.flatten
      user_id = current_user.id
      exam_id = params[:exam_id]
      columns = [:user_id, :exam_id, :answer_id]
      values = answers.map{|answer| [user_id, exam_id, answer]}
  ...
  def score_result
    ...
    question_ids = []
    records.each do |r|
      question_id = r.answer.question_id
      if question_ids.exclude?(question_id) && r.answer.check == true
        score += 1
